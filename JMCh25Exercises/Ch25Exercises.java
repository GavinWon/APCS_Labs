...
public class Ch25Exercises
{
    ...
    {
        int lastNode = (Integer)completeTree[0] / 2;
        for (int index = lastNode + 1; index < completeTree.length ; index++)
        {
            completeTree[index] = null;
        }
        completeTree[0] = lastNode;
    }
    ...
    {
        int Position = 1;
        int leftC = Position * 2 + 1;
        int rightC = Position * 2 + 2;
        
        
        //Keep reheaping down until you can't
        while (leftC <= numNodes && rightC <= numNodes)
        {
            
            if(items[Position] < items[leftC] && items[Position] < items[rightC])
            {
                return;
            }
            else if (items[leftC] < items[rightC])
            {
                Integer temp = items[leftC];
                items[leftC] = items[Position];
                items[Position] = temp;
            ...
            {
                Integer temp = items[rightC];
                items[rightC] = items[Position];
                items[Position] = temp;
        ...
        if (leftC <= numNodes && items[leftC] < items[Position])
        {
            Integer temp = items[leftC];
            items[leftC] = items[Position];
            items[Position] = temp;
                                           ...
                                           PriorityQueue<Message> q2 )
    {
        ...
        while(!q1.isEmpty())
        {
            Message msg1 = q1.remove();
            if (q2.isEmpty())
                output.add(msg1);
            else if (!q2.contains(msg1))
            {
                output.add(msg1);
            }
        }
