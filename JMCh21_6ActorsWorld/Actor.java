...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        while ( !mailbox.isEmpty() )
        {
            ...
            String neededItem = msg.getText().substring( 5 );
            String typeMessage = msg.getText().substring( 0, 4 );
            if ( typeMessage.equals( "need" ) )
            {
                if ( countPossessions( neededItem ) > 1 )
                {
                    reply( msg.getSender(), "have " + neededItem );
                }
            }
            else if ( typeMessage.equals( "have" ) )
            {
                if ( countPossessions( neededItem ) < 1 )
                {
                    reply( msg.getSender(), "ship " + neededItem );
                }
            }
            else if ( typeMessage.equals( "ship" ) )
            {
                if ( countPossessions( neededItem ) > 1 )
                {
                    if ( msg.getSender().receiveItem( this, neededItem ) )
                    {
                        myPossessions.remove( neededItem );
                    }
                }
            }
