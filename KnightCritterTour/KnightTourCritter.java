...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        int lowestAccess = 0;
        for (int index = 0; index <= locs.size() - 1; index++)
        {
            int row = locs.get(index).getRow();
            int col = locs.get(index).getCol();
            if (index == 0)
            {
                bestMove = new Location(row, col);
                lowestAccess = accessGrid[row][col];
            }
            else if (accessGrid[row][col] < lowestAccess)
            {
                bestMove = new Location(row, col);
                lowestAccess = accessGrid[row][col];
    ...
    public void updateAccess( Location loc )
    {
        ...
        for (int row = -2; row <= 2; row++)
        {
            for(int col = -2; col <= 2; col++)
            {
                ...
                //Eliminates all non-L movements
                if ((Math.abs(row) != Math.abs(col)) && row != 0 && col != 0)
                {
                    if(locRow + row < accessGrid.length && locRow + row >= 0 && 
                                    locCol + col < accessGrid.length &&
                                    locCol + col >= 0)
                    {
                        accessGrid[locRow + row][locCol + col] -= 1;
                    }
                }
            }
        }
        accessGrid[locRow][locCol] = -1;
    }
    ...
    public Location selectMoveLocation( ArrayList<Location> locs )
    {
        ...
        Location selectedMove = getBestMove(locs);
        updateAccess(getLocation());

        return selectedMove; // fix this!!!
    }
}
