...
public class JMCh12Exercises
{
    ...
    public boolean[] prob2abcdTrueFalse()
    {
        ...
        // to question (a) - (d)
        // a - false, because new int[100] creates an int array with 100 
        // elements with index 0 - 99.
        // b - true, java syntax allows the array subscript to be any 
        // integer constant, variable, or expression. The result of the 
        // expression just needs to be an int data type.
        // c - true, the index/subscript must always be in the rage of 0 to
        // length - 1; otherwise the program throwns an 
        // ArrayIndexOutOfBoundsException when it is running
        // d - false, any one-dimensional array object has a public field, 
        // length, that returns the size of the array. It is not a method.
    ...
    public boolean allElementsSame( int[] scores )
    {
        ...
        int[] scores3 = Arrays.copyOfRange(scores,0,scores.length - 1);
        return (Arrays.equals(scores2, scores3) && (scores[0] == scores[scores.length - 1]));
    }
    ...
    public char getRandomRps()
    {
        ...
        return rps[index];  // TODO fix this!!
    }
    ...
    public String whatDoesMysteryCountCount()
    {
        ...
        String answer = mCount + " It counts the number of consecective 0s "
            + "in the beginning of the array before it hits any other number"; 
    ...
    public static int[] createWedge(int n)
    {
        ...
        int index = 0;
        for (; index < (wedge.length / 2) ; index++)
        {
            wedge[index] = index + 1;
        }
        wedge[n - 1] = n;
        index++;
        int count = 1;
        for(; index < wedge.length ; index++)
        {
            wedge[index] = n - count;
            count++;
        }
        
        return wedge;  // TODO fix this!!
    }
    ...
    public static int computeScore( String word )
    {
        ...
        {
            int index2 = alphabet.indexOf(word.charAt(index));
            total += letterScores[index2];
        }
        
        return total;  // TODO fix this!!
    }
    ...
    public boolean[] prob11abcdeTrueFalse()
    {
        // return a boolean array with answers corresponding
        // to question (a) - (e)
        // a - true, because ArrayList can hold different multiple references 
        // to the same object, meaning that obj1 == obj2, will result in true
        // boolean
        // b - true, because the same object can belong to different ArrayLists.
        // And by changing that object in one ArrayLists will then change it
        // in the others, unless it is declared immutable.
        // c - false, because although ArrayList removes an object, it does
        // not actually get destroyed. 
        // d - false, because the add method just has adds new reference to the 
        // object
        // e - true, because it is possible for two or more variables 
        // with the same reference to a ArrayList
    ...
    {
        ArrayList<String> revList = new ArrayList<String>(list.size());
        for (int index = 0; index < list.size(); index++)
        {
            revList.add(list.get(list.size() - 1 - index));
        }
    ...
    public void removeMin( ArrayList<Integer> list )
    {
        if (list.size() > 0) {
            ...
            for (int index = 0; index < list.size(); index++)
            {
                int temp = list.get(minIndex).compareTo(list.get(index));
                if (temp == 1)
                {
                    minIndex = index; 
                }
    ...
    {
        Arrays.sort(sample);
        while(sample.length != 1 && sample.length != 2)
        {
            sample = Arrays.copyOfRange(sample, 1 ,sample.length - 1);
        }
        if (sample.length == 2)
        {
            return (m == (sample[0] + sample[1])/2);
        }
        else
        {
            return (m == sample[0]);
    ...
    public static boolean covers( double[][] m1, double[][] m2 )
    {
        ...
        {
            for (int index2 = 0; index2 < m1[index].length; index2++)
            {
                if(m1[index][index2] > m2[index][index2])
                {
                    count++;
                }
                totalElements += 1;
        ...
        return (count >= (totalElements + 1) / 2);   // TODO fix this!!
    }
